...
| 4 | Prod SLO alerts | Continuous regression detection between tests, using the same queries |
...
- **`es.query.duration_ms` + `es.query.took_ms`** — recorded in the
  ...
  `knn`, `popularity`, `author_scan`) supplied by callers; instantiates
...
defined as JSON in this repo (`monitoring/dashboards/bottleneck.json` or
similar) and deployed via `gcloud monitoring dashboards update` — the
...
Alert policies on the prod SLO surface, using the same queries as the
dashboard: `feed.render` p95, failure rate, degraded rate, `freshness_sec`
p95, and `eventloop.lag_ms` p95 once it exists. Alerts carry the
"regression over time" duty; the dashboard carries diagnosis.
...
5. Step 4 alerts — aggressive thresholds on a stage load test first.

**Open questions:**

- `op` label taxonomy for `es.query.*` — exact set and where the label is
  attached (call site vs index-name inference in the wrapper).
- Dashboard JSON home: this repo vs a small `monitoring/` repo (leaning
  this repo; the queries reference api metric names).
- Whether `eventloop.lag_ms` should also gate degradation (shed load when
  saturated) — out of scope here, but the metric enables it.
