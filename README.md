...
`POST /embeddings/user` computes one two-tower user embedding from the same
recent-like history used by the `two_tower` candidate generator, without running
a candidate search. It requires the usual `X-API-Key` authentication:

```bash
curl --fail-with-body http://localhost:8000/embeddings/user \
  -H "X-API-Key: $GE_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"user_did":"did:plc:YOUR_USER_DID"}'
```

A successful response has `status: "ok"`, the numeric `embedding` array,
`model_uuid`, `dimension`, `user_did`, `history_like_count`,
`history_embedding_count`, and `reason: null`. The UUID comes from the prediction
that produced the vector. Only combine vectors from the same model UUID and
dimension; the endpoint does not average or normalize them.

The history counts describe up to 50 recent likes and their usable embeddings
from the `posts` index, rather than the user's total indexed likes. The endpoint
uses the same posts-only history preparation as the production candidate generator;
it does not hydrate liked items from the `replies` index. It reuses the
normal user-history cache, so history may be up to 30 minutes old. Users with no
likes or no usable history embeddings receive HTTP 200 with `status: "skipped"`,
`reason: "no_likes"` or `"no_embedded_history"`, populated history counts, and
null `embedding`, `model_uuid`, and `dimension`. No empty-history fallback is used.

History lookup and inference failures are errors, not skips.
Error responses use `detail: {"code": "...", "message": "..."}`: HTTP 502 with
`upstream_error`, `invalid_inference_response`, or `inference_not_configured`,
and HTTP 504 with `upstream_timeout`. The server uses its configured
