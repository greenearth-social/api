...
Deployment selects the promoted average user embedding from the environment's
model bucket. [Promote an artifact](#inspect-and-promote-an-artifact) before the
first standard deployment, or explicitly pass `--without-average-user-embedding`.
Use `--average-user-embedding-uri` to select an exact artifact for one deployment.
...
- `GE_AVERAGE_USER_EMBEDDING_URI` - Optional exact timestamped GCS artifact override; otherwise deploy resolves the environment's promoted default
...
uploads the selected artifact and prepares an environment's default selection.
Neither command activates an artifact in a feed. The two-tower consumer below
loads the exact artifact selected for its revision. Standard deployments resolve
the promoted default automatically and pin that timestamped artifact URI.
...
the serving models stay fixed during a manual run. The two-tower generator checks
the saved user/post model pair and dimension against the actual prediction before blending.
...
`--project-id` overrides `greenearth-471522` for promotion and deployment; the
...
After promotion, deploy normally with no manually configured embedding environment
variable:

```bash
./scripts/deploy.sh                     # stage
./scripts/deploy.sh --environment prod  # production
```

`deploy.sh` reads `average_user_embeddings/default.json` in the selected environment's
model bucket, downloads and validates its referenced artifact, and logs the run ID,
model pair, dimension, and contributor count. It sets `GE_AVERAGE_USER_EMBEDDING_URI`
to the **timestamped artifact URI** on the new Cloud Run revision. The default must
point to an artifact in the same bucket and prefix, with a filename matching its run ID.

Selection is read-only and happens before feed publication or Cloud Run changes.
Missing, unreadable, or invalid defaults/artifacts stop deployment. Promote an
initial artifact before the first standard deployment. To inspect the selection
without deploying:

```bash
pipenv run python scripts/resolve_average_user_embedding.py --environment stage
```
...
```bash
./scripts/deploy.sh --average-user-embedding-uri \
  gs://your-bucket/average_user_embeddings/average_user_embedding_<run_id>.json

# Intentionally deploy with actual-user-only fallback and no GCS lookup:
./scripts/deploy.sh --without-average-user-embedding
```

The existing `GE_AVERAGE_USER_EMBEDDING_URI` environment override is also supported;
the explicit URI flag takes precedence. An override is validated before deployment
and does not update `default.json`. The disable flag clears an inherited URI and
cannot be combined with the explicit URI flag. Deployment overrides require GCS
URIs; local paths remain available when running the API locally.

The API loads the pinned artifact once per process startup and retains its existing
fallback if that download or validation fails. It does not follow `default.json`.
Promotion alone therefore cannot change existing revisions, including newly started
instances of those revisions. A revision rollback restores its old artifact selection
along with the code, provided that artifact still exists.
For an artifact-only change with the same git SHA, select the revision explicitly
with `scripts/rollback.sh --environment prod --to <revision>`; its automatic previous
deployment selection skips revisions with the current git SHA.

Retain timestamped artifacts for reproducibility and revision rollbacks. To restore
an earlier default for subsequent deployments, promote its immutable artifact again
and redeploy. Never overwrite or delete artifacts referenced by revisions you may
run or roll back to.
...
Standard deployments set `GE_AVERAGE_USER_EMBEDDING_URI` to the resolved immutable
GCS artifact. For local API startup, set it to one exact GCS object or local path:
...
The API validates the normalized version-1 contract, then retains an immutable
copy for the process lifetime. Model-pair and dimension compatibility are checked
against the actual prediction before blending. It loads during startup,
