...
- `GE_PERSPECTIVE_QPM` - Serving's share of the Perspective quota, in requests per minute (default: `27000`). The quota is 36 000 RPM shared with `ingex`, which scores posts at ingest and takes its own slice via `GE_PERSPECTIVE_QPS`; the two should sum to under 36 000
