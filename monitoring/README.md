...
| `es.query.duration − took` gap ↑ on every `op`, `took` flat, `eventloop.lag_ms` flat, CPU well below 100%; `es.client.in_flight` pinned at the pool cap | **ES client connection-pool starvation** (client-side queuing per dependency, not loop-wide) | row 4 gap chart + row 3 lag/CPU + row 3 in-flight chart | #344 cause 1: pool of 10 exhausted by kNN, trivial terms lookups queued to p50 ≈ 918 ms while ES `took` ≈ 10 ms, CPU ≈ 64 % |
...
| 3 — `client.in_flight` / `es.client.in_flight` p95 | **100** | The intended pool caps: `GE_HTTP_MAX_CONNECTIONS` (default 100, `lib/http_client.py`) and the post-#344 `GE_ES_CONNECTIONS_PER_NODE` target of 100. **Caveat:** as of this branch the ES client in `app/main.py` does not yet set a per-node connection limit, so the effective ES cap is the elasticsearch-py default (10), not 100 — the line reads as the target, and `es.client.in_flight` flattening at ~10 is itself the starvation signal until that setting lands. |
