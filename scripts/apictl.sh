...
# apictl - Simple control script for API services
...
start_service() {
    ...
    echo -e "${GREEN}Service $service started (autoscaling: 1-4 instances)${NC}"
}
...
stop_service() {
    ...
    # Switch to manual scaling with 0 instances
    gcloud run services update "$service" \
        --region="$REGION" \
        --no-cpu-throttling \
        --scaling=0 \
...
# Show service status
show_status() {
    echo -e "${BLUE}API Services Status:${NC}"
    echo "===================="
...
main() {
    ...
    case "$command" in
        ...
        "restart")
            ...
            ;;
        *)
            echo "Usage: $0 {start|stop|status|details|logs|restart} [service_name]"
            echo ""
            echo "Commands:"
